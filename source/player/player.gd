...
func _physics_process(_delta: float) -> void:
	...
	if playable:
		...
		_update_hitbox_visibility(GLOBAL.TICK)
	else:
...
func _auto_move_to_go_point() -> void:
	...
	# Compensación de delta para GLOBAL.TICK
	var original_velocity = velocity
	velocity = velocity * (GLOBAL.TICK / get_physics_process_delta_time())
	move_and_slide()
...
func _update_hitbox_visibility(tick: float) -> void:
	...
	_hitbox_mat.set_shader_parameter("visible_amount",
		move_toward(current, target, tick * 8.0))
...
func _tick_shield(tick: float) -> void:

...
func _move_medal(_delta):
	if followingPlayer: _move_towards_player(GLOBAL.TICK)
	else:
		velocity.y += grav * GLOBAL.TICK
		position += velocity * GLOBAL.TICK

func _move_powerup(_delta):
	if powerUpFollowPlayer:
		speed = POWERUP_FOLLOW_SPEED
		_move_towards_player(GLOBAL.TICK)
	else:
		if velocity.y < POWERUP_MOVE_SPEED:
			velocity.y += grav * GLOBAL.TICK
		else: position.x += sin((Time.get_ticks_msec() / 1000.0) * TAU * 1) * GLOBAL.TICK * 100 * randSign
		position += velocity * GLOBAL.TICK

func _move_towards_player(_delta: float) -> void:
	var playerPos = GAME.get_player()
	var direction = (playerPos - position).normalized()
	position += direction * speed * GLOBAL.TICK

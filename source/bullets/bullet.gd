...
# Extends BaseBullet with behavior modifiers
extends BaseBullet
...
var _snd_start_abs: float = 0.0 # fstStartTime + sndStartTime (abs)
...
func _on_acquired() -> void:
	# Reset all modifier state so not to bleed pool reuses
...
func _get_velocity(_delta: float) -> Vector2:
	...
	if directionType == DirectionType.BOUNCE: _handle_bounce()
	
	return final_vel

# ==============================================================================
# PUBLIC API - Called by emitter.gd
# ==============================================================================
...
func modify_direction(
	...
	dirDuration    = newDur
	if directionType == DirectionType.SINE_WAVE: _setup_sine_wave()
...
func _update_direction(delta: float) -> void:
	...
	if _elapsed < dirStartTime or _elapsed > dirStartTime + dirDuration: return
	
	match directionType:
		DirectionType.AIM: direction = (GAME.get_player() - global_position).normalized()
		DirectionType.GRAVITY: useGravity = true
		DirectionType.LEFT: direction = direction.rotated(deg_to_rad(float(deviationAngle)) * delta)
		DirectionType.RIGHT: direction = direction.rotated(-deg_to_rad(float(deviationAngle)) * delta)
		DirectionType.RANDOM:
			direction = direction.rotated(
				deg_to_rad(DRNG.drandf_range(-float(deviationAngle),
				float(deviationAngle))) * delta)

func _update_homing(delta: float) -> void:
	var desired := (GAME.get_player() - global_position).normalized()
	var max_turn := deg_to_rad(float(deviationAngle)) * delta
...
func _update_speed_lerp() -> void:
	if _elapsed >= fstStartTime and _elapsed <= _snd_start_abs: speed = float(fstNewSpeed)
	elif _elapsed > _snd_start_abs: speed = float(sndNewSpeed)

func _update_speed_curve() -> void:
	if _speedCurveDuration <= 0.0: return
	speed = _speedCurve.sample(
		clamp(_elapsed / _speedCurveDuration, 0.0, 1.0)) * _speedCurveBase

func _apply_gravity() -> void:
	acceleration.y = ProjectSettings.get_setting(
		"physics/2d/default_gravity", 980.0) * gravIntensity
...
func _schedule_sub_emitter_delayed() -> void:
	get_tree().create_timer(_subEmitDelay, false).timeout.connect(
		func() -> void:
			if is_instance_valid(self) and !isCancelled: _spawn_sub_emitter(),
			CONNECT_ONE_SHOT
	)

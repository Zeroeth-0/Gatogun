...
func _update(_delta: float) -> void:
	...
	if useGravity:
		_apply_gravity()
		velocity += acceleration * GLOBAL.TICK
	else:
		velocity = direction * speed
	
	# 5. Rotación sprite
	_update_sprite_rotation(GLOBAL.TICK, velocity)
...
func _update_direction(tick: float) -> void:
	...
	match directionType:
		...
		DirectionType.GRAVITY:
			useGravity = true
		DirectionType.LEFT:
			direction = direction.rotated(deg_to_rad(float(deviationAngle)) * GLOBAL.TICK)
		DirectionType.RIGHT:
			direction = direction.rotated(-deg_to_rad(float(deviationAngle)) * GLOBAL.TICK)
		DirectionType.RANDOM:
			direction = direction.rotated(deg_to_rad(DRNG.drandf_range(-float(deviationAngle), float(deviationAngle))) * tick)


func _update_homing(tick: float) -> void:
	if not _is_modifier_active():
		return
	var desired := (GAME.get_player() - global_position).normalized()
	var max_turn := deg_to_rad(float(deviationAngle) * 2.0) * tick
...
func _update_sprite_rotation(tick: float, vel: Vector2) -> void:
	if !has_node("Sprite2D"): return
	match sprite_rotation_mode:
		SpriteRotationMode.SPIN_CONTINUOUS:
			$Sprite2D.rotation_degrees -= rotationSpeed * tick

...
func _update(_delta: float) -> void:
	if BulletType == BulletEnum.FOLLOW: _update_follow(GLOBAL.TICK)
	
	# Lifetime - all times tick down
	_life_remaining -= GLOBAL.TICK
...
func _update_follow(_delta: float) -> void:
	...
	direction = direction.lerp(to_target, 10.0 * GLOBAL.TICK).normalized()
	velocity = direction * speed
	rotation = direction.angle()
...
func _speed_for_type() -> float:
	match BulletType:
		...
		BulletEnum.LASER: return 1500.0

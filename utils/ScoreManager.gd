...
func _physics_process(_delta: float) -> void:
	_tick_hot(GLOBAL.TICK)
	_tick_combo(GLOBAL.TICK)
	_tick_mult(GLOBAL.TICK)
	_tick_medal_countdown(GLOBAL.TICK)

# ==============================================================================
# HOT SYSTEM
# ==============================================================================

func _tick_hot(tick: float) -> void:
	if hot <= 0.0: return
	if _hot_drain_delay > 0.0:
		_hot_drain_delay -= tick
		return
	var prev := hot
	hot = maxf(hot - HOT_DRAIN_RATE * tick, 0.0)
...
func _tick_combo(tick: float) -> void:
	if combo <= 0:
		combo = 0
		return
	
	# Timer only drains while hot is 0
	if hot <= 0.0: _combo_reset_timer -= tick
...
func _tick_medal_countdown(tick: float) -> void:
	if medalCountdown <= 0.0:
		medalCountdown = 0.0
		return
	medalCountdown = maxf(medalCountdown - tick, 0.0)
